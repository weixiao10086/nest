...
const readfile = async (
  obj: isfilesParams,
): Promise<{ data: Map<string, string>; filenum: number }> => {
  return new Promise((resolve, reject) => {
    ...
    let finished = 1;
    const fun = async (
      { filepath, isawait, recursion }: isfilesParams,
      callback?: Function,
    ) => {
...
const generate = (filepath) => {
  return new Promise(async (resolve, reject) => {
    ...
    let filename =
      filenamearr.at(-1) != '' ? filenamearr.at(-1) : filenamearr.at(-2);
    ...
    for (const key of obj.data.keys()) {
      ...
      let valuestr = value.replaceAll(
        /xxx/gi,
        filename.at(0).toUpperCase() + filename.slice(1),
      );
      //去除文档型注释
      ...
      fs.writeFile(path.join(process.cwd(), keystr), valuestr, function (err) {
        if (!err) {
          console.log(
            '文件生成成功，文件名为' + path.join(process.cwd(), keystr),
          );
        }
      });
    }
