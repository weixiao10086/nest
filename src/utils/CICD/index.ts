...
async function deploy() {
  ...
  await new Promise((resolve, reject) => {
    child_process.exec(
      'CD D:/Webqinaduan/nest/nest&npm run build&exit;',
      {
  ...
  await new Promise((resolve, reject) => {
    ...
    fs.copyFileSync(
      ...
      path.join(process.cwd(), '../../../dist/.env.prod'),
    );
    ...
    connect.on('ready', async () => {
      ...
      await new Promise((resolve, reject) => {
        connect.sftp((err, sftp) => {
          ...
          const dest = '/www/wwwroot/dist.zip'; //  linux下存放目录和文件名称。
      ...
      await new Promise((resolve, reject) => {
        ...
        connect.shell((err, stream) => {
          ...
          // 到目录下解压文件，再删除掉zip包
          stream.write('cd /www/wwwroot/ && unzip -o dist.zip \nnext\n');
