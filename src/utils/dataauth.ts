...
export const dataAuth = (
  ...
  for (let i = 0; i < user.roles.length; i++) {
    ...
    if (item.powerkey === 1) {
      ...
      //本级及子级数据
      sqlstring+=` ${sqlstring.length==0?'':'OR'} deptId IN ( SELECT id_descendant FROM dept_closure WHERE id_ancestor = ${item.deptId} )`
      ...
      sqlstring+=` ${sqlstring.length==0?'':'OR'} deptId = ${item.deptId}`
